...
function showResults(list){show(results);const top=list.slice(0,3);$("podium").innerHTML=top.map((p,i)=>`<div class="pod ${i===0?"first":""}"><div class="medal">${["1ST","2ND","3RD"][i]}</div>${avatarMarkup(p,"resultAvatar")}<div class="name">${esc(p.name)}</div><div class="pts">${p.score} TOTAL</div><small>${p.roundWins} round wins</small></div>`).join("");$("otherPlayers").innerHTML=list.slice(3).map(p=>`<div class="otherRow">${avatarMarkup(p,"tiny")}<span>${p.place}. ${esc(p.name)}</span><b>${p.score}</b></div>`).join("")||"<div class='otherRow'>No other players</div>";$("playAgain").classList.toggle("hidden",room?.hostId!==socket.id);startMusic("results")}
